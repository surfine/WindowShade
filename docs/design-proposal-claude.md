...
> **落地状态（2026-09-19，已合入 main 并按“同一版本重新发布”覆盖 1.0.14）**：§2–§11 已实现并有断言，见
...
> **实施偏差（2026-09-22 记录）**：本稿定稿后，用户实机反馈“面板与系统应用名气泡重叠、且气泡与页眉重复写了应用名”。
> 落地版本改为：底 Dock 面板贴近图标（图标上方 4 pt），底部留 38 pt 标签带给系统气泡，页眉不再写应用名
> （单窗口无页眉，多窗口页眉只剩窗口数与显示方式，高 32），卡片在无状态时不留状态行、操作按钮浮在画面右上角。
> 单窗口因此是 **312 × 236 pt**（有状态 312 × 253），而不是本节的 312 × 274；三窗口 912 × 286。
> 其余几何（内边距、间距、圆角、画面 144 pt、操作按钮 28 × 28）与本稿一致。详见 `docs/window-browser-polish.md`。
