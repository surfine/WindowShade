...
一样，换过来的人不用重学。手势、键盘（⌃⌘← / ⌃⌘→）、甩一下都走这一个规则；拐弯仍然占四角。
撤销、换屏排回都照常。

**甩一下标题栏。** iPadOS 26 可以拖住窗口顶部快速甩到一边来排；Mac 上没有。这里用指针拖着标题栏
快速甩出去松手，按松手那一刻的方向走同一架梯子。方向没有照搬 iPad 的“往上全屏”：WindowShade 里
往上一直是收起，同一个标题栏不能有两套相反的规则。只有“窗口确实跟着指针动了、松手时每秒超过
1800 点、没贴着屏幕边”才算：平常拖窗口都会减速再放下，拖文字、拖文件、拖出标签页时窗口不跟着动，
贴边松手是系统自己的拖边平铺。“跟着动”按比例看（至少走了指针一半的路、方向一致），不按固定像素差：
甩得快时窗口会落后指针一两帧，每秒 2400 点时一帧就差 40 点。按下时只查这一扇窗的外框，拖动中只记位置，不拖慢别的 App。
...
方向键走的是同一架梯子，一下到位，浮窗照样出现、说明做了什么：
...
四个快捷键在设置 → 快捷键 → 排布当前窗口里改键或关掉；⌃⌘ 方向键在 Xcode 里是前进、后退，
两边都要用的人可以换一组。

**一本账。** 键盘、手势、窗口浏览排过的窗口记在同一处：用哪种方式排的，捏合或 ⌃⌘↑ 都能撤回；
换了显示器，都会按原来的排法（铺满、半屏、四角）排回去。窗口被人手挪过、改过尺寸，这条记录就作废，
不拿旧位置覆盖人的新安排。
