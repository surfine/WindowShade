...
验证：默认文字大小下设置页截图与改动前**逐像素零差异**（`pixdiff` 报告
`differing pixels=0 maxDelta=0.000`，两张页面），说明观感未变；断言覆盖
“delta 0/-1/-2 相对关系”“默认下仍是 13/12/11/20”。
...
新增离屏入口 `--settings-shots [输出目录]`：用生产设置窗口渲染每一页（浅色/深色），
不需要屏幕录制权限，结果写到 `.build/settings-shots/`（示例归档在
`docs/visual-qa/settings/`）。该入口拍的是主题框架，包含工具栏与内容区；侧栏由系统
材质绘制，离屏不参与渲染，需要侧栏时仍用 `--duo-design-preview` 的整窗截图；效果页的
Metal 预览画布同样不会出现在离屏图里。

逐页外观巡检固化为 `bash scripts/check-settings-appearance.sh`：它跑一遍截图入口，
比较每页内容区的浅色/深色平均亮度（要求浅色 > 0.6、深色 < 0.5、差 ≥ 0.4），结果写入
`.build/design-review/settings-appearance-check.txt`。当前五页实测：

| 页面 | 浅色 | 深色 | 差 |
| --- | --- | --- | --- |
| 效果 | 0.961 | 0.172 | 0.788 |
| 卷帘 | 0.963 | 0.168 | 0.794 |
| 窗口浏览 | 0.939 | 0.195 | 0.744 |
| 权限与启动 | 0.983 | 0.141 | 0.842 |
| 高级 | 0.964 | 0.165 | 0.799 |
