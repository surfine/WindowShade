...
- 默认窗口 900 × 680，最小 820 × 580；侧边栏初始 214；内容列上限 640、在详情区**居中**（左右至少 28、上下 22）。
  窗口的最小尺寸只写在 `window.contentMinSize` 上：给 `splitController.view` 挂 required 的宽/高约束，
  AppKit 会在展开侧栏时靠"把整扇窗口撑大一个侧栏宽度"来满足它（实测 900 → 1115 pt），切换侧栏因此
  会改变窗口尺寸。居中而不是贴左边：收起侧栏后详情区变成整窗宽，贴左边会在右半边留下近半扇窗的空白。Apple 自己的分组表单
  也是这个摆法（本机实测：854 pt 容器左右各 75 pt、1200 pt 容器左右各 248 pt，内容列恒为 703.5 pt）。
- 四页和引导权限区复用 `NSBox.custom` 分组盒：系统实色、圆角 12（`SystemCornerRadius.card`）、无描边无阴影。分组标题纯文字，分隔线左端缩进 16。
...
- 标准标题栏使用 popover 材质、顶边高光及 0.5pt 内边；置顶标题使用 HUD 材质，悬停时出现。预览缩略图圆角按同心规则取“面板 13 − 内边距 10 = 4”，见《圆角刻度》一节。
...
现在前 9 个内联并带 ⌃⌘1…⌃⌘9，其余进「更多收起的窗口（N）」子菜单（同样的动作、图标
...
## 圆角刻度：跟 macOS 27 的窗口对齐（1.0.14 之后）

在此之前每个表面各写各的数字（面板 16、卡片 12、图片 8、经典条直角、设置卡片 10、
chip 3），曲线也一半正圆一半连续曲率，所以同一屏里会出现“更圆的窗口 + 方角的画面”
这类不顺眼。现在所有自定义表面读同一份 `SystemCornerRadius`：

- **窗口级 13 pt。** 本机实测 macOS 27 的标准窗口：`screencapture -l` 抓 Finder 与
  ChatGPT 窗口，左上角弧长都是 26 px @2x = 13 pt，且轮廓比正圆更平（连续曲率）。
  浮窗、卷帘条、置顶预览、悬停预览都取这个值。
- **内容级 12 pt。** 窗口浏览卡片、设置页分组盒、引导页行统一 12 pt。
- **控件级 6 pt。** 自绘 chip / 无边框按钮。
- **同心嵌套。** 卡片里的画面取“卡片圆角 − 卡片内边距”（12 − 8 = 4 pt），悬停预览
  的画面取“13 − 10 = 4 pt”，最小值 4 pt 以免在 8–10 pt 的间距下退化成直角。这条
  规则出自 HIG：Live Activities 写明“内层圆角 = 外层圆角 − 边距”，Widgets 与
  Toolbars 同样要求内容与控件和外框同心。
- **连续曲率。** 所有图层圆角统一 `.continuous`；`NSBezierPath` 只能画正圆角，所以
  卷帘条这类路径绘制改用 `SystemCornerPath`（按 0.4477·r 的控制点画连续曲率），
  截图的圆角裁切走同一函数的 CGPath 版本。
- **卷帘条的形状。** 经典卷帘条从“整块直角”改为“上面两角跟随窗口圆角、下边缘是
  直切口”，与截图条（真实窗口 chrome 自带圆角）一致；框线与纸面阴影用同一条路径。
- **Liquid Glass 仍在操作层。** 依据 HIG 的 Materials 一节，玻璃只用于控制/导航层，
  内容层用标准材质；本次只统一形状与节奏，没有把玻璃铺进内容。

回归覆盖：`tests/run-paper-tests.sh` 断言刻度值、同心公式、条状路径的下边缘是直线、
上两角是圆角、以及图层确实使用 `.continuous`；`tests/run-window-browser-tests.sh`
断言面板/卡片/画面的圆角关系（包含放大系统字号后仍然成立）。

## 面板贴合内容：不留“下巴”（1.0.14 之后）

用户指出单窗口 Dock 面板在卡片下方留着一大块空白。查下来是三处**无条件预留**叠在一起
（面板本来就是“内容决定尺寸”，这三处都不该无条件保留）：

1. `chromeHeight` 多算了一圈 `panelPadding * 2`（24 pt）。内容视图的布局只消费
   页眉 + 间隔 + 内容 + 页脚，这 24 pt 没有任何人使用，于是永远落在卡片下方。
2. 页脚（19 pt + 8 pt 间隔）即使没有状态文字也占位。
3. 卡片标题无条件按两行预留（34 pt），短标题下面空一整行。

改法（A+C）：

- **页脚只在有状态文字时占位**：`WindowBrowserLayoutParams.footerVisible`，没有状态时
  底部只留一层 `panelPadding`（12 pt）。
- **标题按实际行数取高**：`WindowBrowserGeometry.titleLines(forTitles:…)` 量一次标题
  宽度（结果带缓存，200 条约 0.02 ms），只有真会换行的标题才占两行；
  `derivedParams(base:titles:hasStatus:)` 把这两条派生规则收在一处，控制器与截图探针
  共用，真机与归档图不会再各算一套。
- **面板高度 = 页眉 + 页眉下的间隔 + 内容 + 底部那一块**，不再有无人消费的余量：
  `content.listRect.height == documentHeight` 成为断言。
- 状态出现/消失导致高度变化时走 `setPanelFrame(_:animated:)`，减少动态效果下不动画。

同机实测（生产组件截图，2x）：

| 场景 | 改前 | 改后 |
| --- | --- | --- |
| 单窗口 · 短标题 · 无状态 | 312 × 329 pt，卡片下方 51.5 pt 空档 | **312 × 236 pt**：卡片 186 pt + 12 pt 内边距 + 38 pt 标签带（菜单/快捷键面板没有标签带） |
| 单窗口 · 短标题 · 有状态（“正在刷新窗口…”） | — | 312 × 253 pt（状态行真的存在时才长出来） |
| 三窗口 · 两行网格 · 有页脚 | 612 × 575 pt | 612 × 519 pt（底 Dock 单行三列为 912 × 286 pt） |
...
| 浮动纸面、标题与缩略图 | 统一材质、窗口级 13pt / 缩略图同心 4pt 圆角、0.5pt边缘、1像素高光和共享阴影；组件状态检查、阴影生命周期检查 |
