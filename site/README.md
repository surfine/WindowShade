...
本地预览地址 `http://127.0.0.1:4318`；端口占用时可用 `PORT=4319 npm run dev`。部署前运行构建与检查；首次登录使用 `npx wrangler login`。项目存在后运行：
...
- `scripts/content.mjs`：中英文文案；`scripts/build.mjs`：页面模板、SEO、404、安全响应头。
  ...
  看一眼、带到每张桌面、标题栏手势、置顶、窗口浏览、合盖彩蛋（可拖动滑块合盖、切换三种质感）、快捷键、隐私与问答。
...
- `app.js`：主题切换、滚动后页眉细线、首次进入的轻量浮现；Hero 收起/展开与拖动（按下即捕获指针，边缘回弹）、
  随滚动收起/展开；三种方式的对比动画；看一眼、置顶与窗口浏览示意；合盖动画。演示不操纵真实窗口；
  窗口浏览示意默认展开、指针移开收起、点一下留着，和真实面板一样只读；合盖动画随滚动推进，操作滑块后由读者控制。
  看一眼支持悬停、跨缝移入卡片、触屏轻点、键盘开合和 Esc 收回；读屏状态与画面同步。
  所有动效在“减少动态效果”下退回静态或短淡入。
...
依据原手册重绘的界面均标注为示意。年代以原始手册和 Apple 发布资料核对；1989–92 版权区间不作为首发日期，2001 OS X 和 2003 Exposé 分开。14 条史料与致谢随页面交付，不部署下载的研究 PDF。
...
构建目录共 36 个文件，合计 855,408 字节（未压缩文件内容，不是网络传输量）。中文首页 32,100 字节、英文首页 33,002 字节、共用 CSS 45,485 字节、共用脚本 26,088 字节。旧系统模拟器仍按读者点击后才加载，不计入本地发布载荷。
