...
const download = (c, extra = '') => `<a class="button primary ${extra}" href="${release}">${c.download}<span aria-hidden="true">↗</span></a>`;
function page(c) {
  return `<!doctype html>
<html lang="${c.lang}">
<head>
...
<header class="header"><a class="brand" href="${c.path}" aria-label="WindowShade"><img src="/media/icon.png" width="36" height="36" alt=""><span>WindowShade</span></a><nav aria-label="${c.lang === 'en' ? 'Main navigation' : '主导航'}"><a href="#experience">${c.nav[0]}</a><a href="#pinning">${c.lang === 'en' ? 'Pin a window' : '窗口置顶'}</a><a href="${c.lang === 'en' ? '/en/history/' : '/history/'}">${c.lang === 'en' ? 'Window stories' : '窗口往事'}</a></nav><div class="header-tools"><a class="language" href="${c.languageHref}" lang="${c.lang === 'en' ? 'zh-CN' : 'en'}">${c.language}</a><button type="button" class="theme" aria-label="${c.theme}" title="${c.theme}"><span aria-hidden="true">◐</span></button><a class="nav-download" href="${release}">${c.download}<span aria-hidden="true"> ↗</span></a></div></header>
<main id="main">
<section class="hero wrap"><div class="hero-copy"><p class="eyebrow">${c.eyebrow}</p><h1>${c.hero}</h1><p class="hero-intro">${c.intro}</p><div class="actions">${download(c)}<a class="text-link" href="#experience">${c.try}<span aria-hidden="true">↘</span></a></div></div><div class="hero-art"><img src="/media/hero.webp" width="1536" height="1024" alt="${c.lang === 'en' ? 'Sculptural white sheets rolling into slender shade bars' : '白色纸面轻轻卷成细长卷帘的品牌作品'}" fetchpriority="high"></div></section>
<div class="facts wrap">${c.facts.map(f => `<span>${f}</span>`).join('')}</div>
<section class="experience wrap section" id="experience"><div class="section-copy"><h2>${c.demoTitle}</h2><p>${c.demoBody}</p><div class="demo-buttons"><button class="button secondary" id="fold" type="button" aria-controls="reference-body" aria-expanded="true" data-fold="${c.fold}" data-unfold="${c.unfold}">${c.fold}<span aria-hidden="true">↑</span></button><button class="text-link pin-button" id="pin" type="button" aria-pressed="false" data-pin="${c.pin}" data-unpin="${c.unpin}">${c.pin}</button></div><noscript><p>${c.lang === 'en' ? 'Enable JavaScript to try the interactive illustration.' : '开启 JavaScript 即可体验交互示意。'}</p></noscript></div>
<div class="demo-column"><div class="desk" id="desk"><div class="draft"><div class="draft-label">${c.draftTitle}</div><h3>${c.draftHeading}</h3><p>${c.draftText}</p><div class="writing-lines" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div></div><div class="reference" id="reference"><button type="button" class="reference-bar" id="reference-bar" aria-expanded="true" aria-controls="reference-body" aria-label="${c.noteTitle}: ${c.fold}"><span class="traffic" aria-hidden="true"><i></i><i></i><i></i></span><span>${c.noteTitle}</span><span aria-hidden="true" class="bar-glyph">⌃</span></button><div class="reference-body" id="reference-body"><div class="note-rule"></div><h3>${c.noteHeading}</h3><p>${c.noteBody}</p><div class="note-footer">${c.noteFooter}</div></div><div class="paper-roll" aria-hidden="true"></div></div></div><p class="caption">${c.demoCaption}</p><p class="sr-only" id="demo-status" aria-live="polite" data-expanded="${c.expandedStatus}" data-folded="${c.foldedStatus}" data-pinned="${c.pinnedStatus}"></p></div></section>
<section class="product-detail wrap section" id="compare"><h2>${c.comparisonTitle}</h2><p class="detail-intro">${c.comparisonIntro}</p><div class="comparison-scroll" role="region" aria-label="${c.lang === 'en' ? 'Compare window actions' : '窗口管理方式对照'}" tabindex="0"><table class="comparison-table"><thead><tr>${c.comparisonHeads.map(h => `<th scope="col">${h}</th>`).join('')}</tr></thead><tbody>${c.comparisonRows.map(row => `<tr>${row.map((cell, i) => i === 0 ? `<th scope="row">${cell}</th>` : `<td>${cell}</td>`).join('')}</tr>`).join('')}</tbody></table></div></section>
<section class="features wrap section"><h2>${c.featureTitle}</h2><div class="feature-grid">${c.features.map(([title, body, key]) => `<article><kbd>${key}</kbd><h3>${title}</h3><p>${body}</p></article>`).join('')}</div></section>
<section class="pinning wrap section" id="pinning"><div class="pinning-intro"><div><p class="eyebrow">⌃ ⌘ P · ${c.lang === 'en' ? 'KEEP IT VISIBLE' : '窗口置顶'}</p><h2>${c.pinTitle}</h2><p class="detail-intro">${c.pinIntro}</p></div><div><div class="pin-desk" id="pin-desk"><div class="pin-source"><div class="pin-chrome">${c.pinDemoTitle}</div><strong>${c.pinDemoBody}</strong></div><div class="pin-work"><div class="pin-chrome">${c.pinWork}</div><p>${c.pinWorkBody}</p><div class="writing-lines" aria-hidden="true"><i></i><i></i><i></i></div></div></div><div class="demo-buttons"><button class="button secondary" id="pin-example" type="button" aria-pressed="false" data-on="${c.pinUndo}" data-off="${c.pinAction}">${c.pinAction}</button><button class="text-link" id="pin-switch" type="button">${c.pinSwitch} ↗</button></div><p class="pin-state" id="pin-state" aria-live="polite" data-on="${c.pinStatusOn}" data-off="${c.pinStatusOff}">${c.pinStatusOff}</p><p class="caption">${c.pinDemoCaption}</p></div></div><div class="pin-steps">${c.pinSteps.map(([title,body],i) => `<article><span class="pin-step-number">0${i+1}</span><h3>${title}</h3><p>${body}</p></article>`).join('')}</div><p class="pin-pair">${c.pinPair}</p><p class="mechanics-note">${c.pinNote}</p><p class="caption"><a href="https://github.com/lihaoyun6/Topit">${c.pinCredit} ↗</a></p></section>
<section class="product-detail wrap section" id="styles"><h2>${c.mechanicsTitle}</h2><p class="detail-intro">${c.mechanicsIntro}</p><div class="style-options">${c.styles.map(([title,body]) => `<article><h3>${title}</h3><p>${body}</p></article>`).join('')}</div><p class="mechanics-note">${c.mechanicsFoot}</p><a class="text-link" href="${repo}/blob/main/WindowShade.md">${c.researchLink} ↗</a></section>
...
<section class="settings wrap section"><div class="settings-art"><picture><source srcset="/media/settings-dark.webp" media="(prefers-color-scheme: dark)"><img id="settings-image" src="/media/settings.webp" loading="lazy" width="1440" height="1088" alt="${c.settingsCaption}"></picture></div><div class="section-copy"><h2>${c.settingsTitle}</h2><p>${c.settingsBody}</p><p class="caption">${c.settingsCaption}</p></div></section>
<section class="trust wrap section" id="privacy"><h2>${c.trustTitle}</h2><div class="trust-list">${c.trust.map(([title, body]) => `<article><h3>${title}</h3><p>${body}</p></article>`).join('')}</div></section>
...
<section class="history-teaser wrap"><div><p class="dateline">WINDOWSHADE FIELD NOTES · 001</p><h2>${c.lang === 'en' ? 'After a window moves aside,<br>what stays with you?' : '窗口让开以后，<br>你还记得它在哪里吗？'}</h2></div><div><p>${c.lang === 'en' ? 'An interactive archaeology of a small gesture. Change a 1994 preference, move a collapsed window, and boot an actual old Mac.' : '一个小动作的互动考古。改动 1994 年的点击偏好，挪动一条收起的标题栏，再亲手启动一台真正的旧 Mac。'}</p><a class="text-link" href="${c.lang === 'en' ? '/en/history/' : '/history/'}">${c.lang === 'en' ? 'Explore the window stories' : '翻开窗口往事'} ↗</a></div></section><section class="closing wrap section"><img src="/media/icon.png" width="72" height="72" alt="" loading="lazy"><h2>${c.endTitle}</h2><p>${c.endBody}</p>${download(c)}</section>
