...
<section class="feature glancing wrap" id="glance">
<div class="feature-copy"><p class="kicker">${c.glanceKicker}</p><h2>${c.glanceTitle}</h2><p class="lead">${c.glanceIntro}</p><ul class="points">${c.glancePoints.map(([title, detail]) => `<li><strong>${title}</strong><span>${detail}</span></li>`).join('')}</ul><p class="note">${c.glanceHow}</p></div>
<div class="feature-demo"><div class="stage stage-glance"><div class="stage-menubar" aria-hidden="true"><b>${c.stageMenu[0]}</b>${c.stageMenu.slice(1).map(m => `<span>${m}</span>`).join('')}<span class="stage-clock">9:41</span></div><div class="win win-draft inactive" aria-hidden="true"><div class="win-bar">${lights}<span class="win-title">${c.draftTitle}</span></div><div class="win-body"><p class="win-kicker">${c.draftKicker}</p><h3>${c.draftHeading}</h3><div class="writing-lines"><i></i><i></i><i></i></div></div></div><button type="button" class="win glance-strip" aria-expanded="false" aria-controls="glance-card" aria-label="${c.glanceStripLabel}"><span class="win-bar">${lights}<span class="win-title">${c.noteTitle}</span></span></button><div class="glance-card" id="glance-card" aria-hidden="true"><div class="win-body"><p class="win-kicker">${c.noteKicker}</p><h3>${c.noteHeading}</h3><p>${c.noteBody}</p></div></div></div><p class="caption">${c.glanceCaption}</p></div>
</section>

<section class="feature swiping wrap" id="gestures">
<div class="feature-copy"><p class="kicker">${c.swipeKicker}</p><h2>${c.swipeTitle}</h2><p class="lead">${c.swipeIntro}</p><ul class="points">${c.swipePoints.map(([title, detail]) => `<li><strong>${title}</strong><span>${detail}</span></li>`).join('')}</ul><p class="note">${c.swipeHow}</p></div>
<div class="feature-demo"><div class="stage stage-swipe" id="swipe-desk" data-state="normal" role="group" aria-label="${c.swipeLabel}"><div class="stage-menubar" aria-hidden="true"><b>${c.stageMenu[0]}</b>${c.stageMenu.slice(1).map(m => `<span>${m}</span>`).join('')}<span class="stage-clock">9:41</span></div><div class="win swipe-win" id="swipe-win"><span class="cast" aria-hidden="true"></span><div class="win-bar" id="swipe-bar">${lights}<span class="win-title">${c.swipeWindow}</span><span class="swipe-fingers" aria-hidden="true"><i></i><i></i></span></div><div class="win-body" aria-hidden="true"><p class="win-kicker">${c.swipeWindowKicker}</p><h3>${c.swipeWindowHeading}</h3><div class="writing-lines"><i></i><i></i><i></i><i></i></div></div></div><div class="gesture-hud swipe-hud" id="swipe-hud" aria-hidden="true" data-titles='${JSON.stringify(c.swipeHud).replace(/'/g, '&#39;')}'><span class="hud-title" id="swipe-hud-title"></span><span class="hud-row"><i class="hud-from"></i><span class="hud-track"><b id="swipe-hud-fill"></b></span><i class="hud-to"></i></span></div></div><div class="demo-buttons"><button class="chip" type="button" data-swipe="up">${c.swipeUp}</button><button class="chip ghost" type="button" data-swipe="down">${c.swipeDown}</button></div><p class="demo-state" id="swipe-state" aria-live="polite" data-states='${JSON.stringify(c.swipeStates).replace(/'/g, '&#39;')}'>${c.swipeStates.ready}</p><p class="caption">${c.swipeCaption}</p></div>
</section>
