...
type State = {
  rect: Rect;
  roll: number;
  hud?: { title: string; p: number; o: number; armedAt?: number };
  fingers: { x: number; y: number; on: number };
};
...
function stateAt(f: number): State {
  ...
  if (f < G.expand.down - 10) {
    ...
    return {
      ...
      hud: {
        title: "收起窗口",
        p,
        o: touching(f, g) + (f > g.release ? 1 - tw(f, g.release, g.release + 10) : 0),
        armedAt: g.release - 12,
      },
      fingers: {
        x: 0.5,
        y: 0.66 - Math.min(p, 1.12) * 0.3,
        on: touching(f, g),
      },
    };
  }
  ...
  const place = (
    ...
    return {
      ...
      fingers: {
        x: fx(Math.min(p, 1.12)),
        y: fy(Math.min(p, 1.12)),
        on: touching(f, g),
      },
    };
  };
...
function cancelOrWheel(f: number, rest: { x: number; y: number; on: number }): State {
  if (f < G.wheel.swap) {
    const g = G.cancel;
    const p = interpolate(f, [g.down + 6, g.peak, g.back], [0, 0.8, 0], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
      easing: Easing2,
    });
    ...
    return {
      ...
      hud: {
        title: "收起窗口",
        p,
        o: on * (f > g.back - 12 ? 1 - tw(f, g.back - 12, g.back + 2) : 1),
      },
  ...
  return {
    ...
    hud: {
      title: "铺满屏幕",
      p: steps >= 0.999 ? 1 : steps,
      o: hudOn,
      armedAt: w.notches[2] + 1,
    },
    fingers: { ...rest },
  };
}
...
export const Gestures: React.FC = () => {
  ...
  const v = useVertical();
  // 9:16: the Mac shrinks to 80% at the top, the hand goes underneath.
  const macFit: React.CSSProperties = v
    ? {
        position: "absolute",
        inset: 0,
        transformOrigin: "0 0",
        transform: `translate(${56 - SCREEN.x * 0.8}px, ${470 - SCREEN.y * 0.8}px) scale(0.8)`,
      }
    : { position: "absolute", inset: 0 };
  const hand = v ? { x: 290, y: 1190, label: 1545 } : { x: 50, y: 500, label: 860 };
      ...
      <Canvas dark>
        <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
          ...
          <Rise at={G.line.at + 60}>
            ...
            >
              Now try two fingers on the title bar.
            </div>
          </Rise>
        </AbsoluteFill>
      </Canvas>
  ...
  const barCenter = (st: State) => ({
    x: SCREEN.x + st.rect.x + st.rect.w / 2 + 60,
    y: SCREEN.y + st.rect.y + BAR / 2 + 2,
  });
  ...
  return (
    <Canvas>
      ...
      <Caption
        zh={"往下拉，展开；\n再拉，铺满屏幕。"}
        en="Pull down to unroll it, again to fill the screen."
        at={G.expand.down - 14}
        out={G.left.down - 26}
      />
      <Caption zh="左右滑，各占半屏。" en="Swipe sideways for half the screen." at={G.left.down - 14} out={G.cancel.down - 22} />
      <Caption zh={"滑到一半往回拉，\n就当没发生过。"} en="Pull back halfway and nothing happens." at={G.cancel.down - 12} out={G.wheel.swap - 14} />
      ...
      <div
        style={{
          position: "absolute",
          inset: 0,
          ...tiltIn(frame, G.desk, { y: 220, rx: 26, ry: -8 }),
        }}
      >
        <div style={macFit}>
          <div
