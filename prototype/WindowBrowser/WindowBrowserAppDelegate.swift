...
extension AppDelegate {
    ...
    @discardableResult
    func registerWindowBrowserFoldWaiter(id: CGWindowID,
                                         completion: @escaping (Bool) -> Void) -> UUID {
        let token = UUID()
        windowBrowserFoldWaiters[id, default: [:]][token] = completion
        // 兜底：折叠事务异常中止且没有走到任何结算点时，避免等待闭包长期驻留。
        DispatchQueue.main.asyncAfter(deadline: .now() + 30) { [weak self] in
            self?.settleWindowBrowserFoldWaiter(id: id, token: token, success: false)
        }
        return token
    }

    /// 只结算指定 token，避免同一窗口后来的一次折叠被旧超时误结算。
    func settleWindowBrowserFoldWaiter(id: CGWindowID, token: UUID, success: Bool) {
        guard var waiters = windowBrowserFoldWaiters[id],
              let completion = waiters.removeValue(forKey: token) else { return }
        if waiters.isEmpty {
            windowBrowserFoldWaiters.removeValue(forKey: id)
        } else {
            windowBrowserFoldWaiters[id] = waiters
        }
        completion(success)
    }
    ...
    func settleWindowBrowserFoldWaiters(id: CGWindowID, success: Bool) {
        guard let waiters = windowBrowserFoldWaiters.removeValue(forKey: id) else { return }
    ...
    func windowBrowserBeginFold(key: WindowKey, element: AXUIElement,
        ...
        let token = registerWindowBrowserFoldWaiter(id: id, completion: completion)
