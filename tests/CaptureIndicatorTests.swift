...
enum CaptureIndicatorTests {
    ...
    static func purpleCount(_ image: CGImage, scale: CGFloat) -> Int {
        ...
        let pixels = RGBAPixels(image.cropping(to: CGRect(x: 0, y: 0, width: w, height: h))!)!
        var count = 0
        for y in 0..<h { for x in 0..<w where pixels.isIndicatorPurple(x: x, y: y) { count += 1 } }
        return count
    }
    ...
    static func main() {
        ...
        if failures == 0 {
            print("PASS: capture indicator removed from real active/inactive samples; plain, dark and purple-toolbar title bars untouched")
        } else {
