...
- **窗口浏览的成熟版。** 单窗口面板约 312×349 pt，不再留一大片空白；窗口截图与标题优先，动作收进悬停才出现的紧凑符号条；状态改用系统符号，不再出现 emoji 或异常警告色。
- **系统材质。** macOS 26 及以上使用公开的 AppKit Liquid Glass 控制层，旧系统使用系统原生材质；纸面、减少透明度、提高对比度与减少动态效果都有明确回退。最低运行目标仍是 macOS 14。
- **排布与撤销（新）。** 卡片右键可以对窗口执行左半/右半/四角/居中/填满可用区域/移到另一显示器，先预览再执行；验证成功才登记撤销，手动移动过窗口后撤销会被拒绝。
- **动作绑定真实身份。** 每个卡片动作都绑定你所选窗口的完整身份，执行前重新核对；折叠与展开复用原有事务、恢复验证与恢复日志。
- **调度与资源。** 截图任务按编号记账，取消但仍在系统里执行的截图继续占额度直到真实完成；订阅有明确终态，实时画面只有一个挂载点；120 个窗口首次填充从约 200 ms 降到约 18 ms。

[发布说明与下载 →](https://github.com/surfine/WindowShade/releases/tag/v1.0.13)
