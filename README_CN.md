...
- **面板贴着内容收口。** 标题按实际行数取高，没有状态文字时页脚不占位：单窗口 Dock 面板从 312 × 329 pt 收到 312 × 274 pt，卡片下方只留 12 pt 内边距，不再是一块“下巴”。
- **液态玻璃按 HIG 的分层纪律用。** 整个应用只保留一层 `NSGlassEffectView`（Apple 给文字多的弹窗/侧栏规定的 `regular` 档），控制层不再叠第二层玻璃，卡片与列表行改用内容层标准材质；玻璃不上色，最低运行目标仍是 macOS 14，并保留不透明、提高对比度与减少动态效果的回退。
