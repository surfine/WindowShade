...
**在标题栏上滑一下。** 指针停在标题栏上，两指往上一推，窗口像卷帘一样收起来；往下一拉，窗口铺满菜单栏和 Dock 之间的屏幕，再往上推又回到原来的大小。在卷帘条上往下拉就展开。左右滑占半屏；两指像放大照片那样张开也是铺满，捏合放回原处。轻点两下（触控板两指、Magic Mouse 单指）在铺满和还原之间切换。普通鼠标滚轮三格算一次；Magic Mouse 已加入单指滑动和轻点两下的适配，仍待真机验证。手指移动时，窗口旁边会出现一块和系统音量提示同款的小浮窗，告诉你松手会做什么、还差多少；往回拉就取消。Safari 标签上的左右滑照样切换标签，Swish 在运行时标题栏手势交给它。细节见 [标题栏手势说明](docs/gestures.md)。
...
- **1.0.15 下载包：** ZIP 3.99 MB，解压后的应用文件合计 7.72 MB（十进制；文件系统实际占用可能不同）。
