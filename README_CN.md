...
`⌃⌘C` 或双击标题栏，把窗口内容收成一条卷帘条：位置、标题和回来的入口都还在。单击卷帘条预览内容，再点展开。`⌃⌘1…9` 按菜单顺序展开，`⌃⌘0` 把卷帘条排整齐或进入专注布局。两种样式（原貌截图 / 标准标题栏），支持半透明、折叠音效和登录启动。

### 置顶预览

`⌃⌘P` 把窗口变成始终可见的实时预览。它跟着源窗口一起走，空闲时自动降频省电，适合参考资料、镜像画面和仪表盘。

### 动态效果

屏幕合拢时，桌面或窗口内容会随铰链角度卷起、后退、模糊，再原样展开。三种质感（轻柔 / 标准 / 磨砂）、触发角度可调、预览进度能拖着看；“随设备倾斜”是实验功能，会给桌面效果加一点随机器姿态变化的视差。
