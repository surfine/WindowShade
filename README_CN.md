...
- **一套外观策略，管住所有表面。** 卷帘条、悬停缩略图、置顶预览、窗口浏览面板、设置与引导页读的是同一份系统外观策略；动态颜色改在视图自己的外观里解析，深色下不会再出现“浅色行 + 深色面板”，经典卷帘条也会跟着系统切换重算配色。
- **补齐菜单栏应用该有的 AppKit 契约。** 真正的主菜单（⌘X/⌘C/⌘V/⌘A/⌘Z、⌘W、关于），协作式激活，设置、引导与面板不再被系统并成标签页，权限深链优先跳新的 ExtensionKit 面板。
- **可访问性补到能操作。** 卡片、列表行、卷帘条与预览共用同一份能力模型；VoiceOver 焦点跟随方向键与搜索，VO-Space 与鼠标点击走同一条激活路径。
- **Space 看一眼原窗口。** 只读大图预览，不激活、不展开、不移动窗口；点击或 Escape 关闭，Escape 依次处理大图、排布预览与面板。
- **依然快，而且数据诚实。** 120 个窗口首次填充与布局 24.2 / 30.4 ms（p50/p95），同一次会话内背靠背测得的基线是 271.8 / 342.8 ms；暖刷新仍在 4 ms p95 预算内。
- **圆角跟系统一致。** 所有自定义表面读同一份刻度：窗口与面板 13 pt（在本机真实窗口上量得）、卡片 12 pt、控件 6 pt，一律连续曲率；卡片里的画面按 HIG 的同心规则取“外圆角 − 内边距”。卷帘条也从整块直角改成上面两角圆、下边缘直切，像它替代的那扇窗口。
- **面板贴着内容，也贴着 Dock 图标。** 标题按实际行数取高，没有状态的窗口不留状态行，操作按钮浮在画面右上角而不再独占一行：单窗口 Dock 面板 312 × 236 pt，卡片下方只留 12 pt 内边距，1.0.13 的同一面板是 312 × 329 pt。底部 Dock 的面板贴近图标（上方 4 pt），最下方留 38 pt 标签带给系统的应用名气泡；气泡收起后由面板自己的标签接替，应用名不会写第二遍。从菜单或快捷键打开的键盘面板仍保留完整页眉。
- **液态玻璃按 HIG 的分层纪律用。** 整个应用只保留一层 `NSGlassEffectView`（Apple 给文字多的弹窗/侧栏规定的 `regular` 档），控制层不再叠第二层玻璃，卡片与列表行改用内容层标准材质；玻璃不上色，阴影改用系统窗口阴影，让阴影四角与玻璃圆角重合；最低运行目标仍是 macOS 14，并保留不透明、提高对比度与减少动态效果的回退。

[发布说明与下载 →](https://github.com/surfine/WindowShade/releases/tag/v1.0.14)
...
| **浏览 Dock 图标** | 鼠标停在 Dock 的应用图标上，弹出该应用的真实窗口面板——缩略图卡片或紧凑列表，可搜索、可对单个窗口操作。菜单里的“选择窗口…”或自设快捷键打开同一个键盘面板，默认关闭。 | 记得窗口长什么样，却不记得它在哪。 |
...
窗口浏览是临时面板：它不替换系统 Dock，也不接管原生 Command-Tab。
...
| 按菜单顺序展开 | `⌃⌘1…9` |
| 整理卷帘条 / 专注布局 | `⌃⌘0` |
| 打开窗口浏览 | 菜单 → “选择窗口…”；默认不占用快捷键 |

表里的组合是默认值。设置 → 快捷键里可以逐个改键或关掉，`⌃⌘1…9` 整组开关；
组合被别的应用占用时会提示一次，菜单里也不再显示它。
